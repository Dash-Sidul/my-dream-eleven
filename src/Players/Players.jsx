...
const Players = ({players, handleChosePlayer, active, chosenPlayer, available, handleAvailable, selected, handleSelected , purchasePlayer, setPurchasePlayer, block, setBlock}) => {
    ...
    return (
        <div>
          ...
          <div className=''>
            <h1 className="text-2xl font-bold">{selected ? 'Selected Player': 'Available Player'}</h1>
          </div>
          <div>
        ...
        <div className={`grid grid-cols-3 gap-4 mt-5 ${!available ? 'hidden':''} ${selected ? 'hidden' : ''}`}>
            {
